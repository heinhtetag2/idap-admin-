...
export default function Surveys() {
  ...
  const getStatusBadge = (status: SurveyStatus) => {
    switch (status) {
      case 'Active':    return 'bg-[#E8F5E9] text-[#2E7D32] border border-[#C8E6C9]';
      case 'Draft':     return 'bg-[#F5F2F0] text-[#8A8284] border border-[#EAE5E3]';
      case 'Paused':    return 'bg-[#FFF3E0] text-[#E65100] border border-[#FFE0B2]';
      case 'Completed': return 'bg-[#E5F0FF] text-[#1F5C8C] border border-[#D9E2FF]';
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-serif text-[#2C2627]">{t('Surveys')}</h1>
          <p className="text-sm text-[#8A8284] mt-1">
        ...
        {[
          {
            ...
            trend: <span className="text-[#2E7D32] flex items-center gap-0.5"><ArrowUpRight className="w-3 h-3" /> 12%</span>,
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <span className="text-sm font-medium text-[#8A8284]">{t(card.title)}</span>
              <div className="p-2 bg-[#F5F2F0] rounded-md text-[#5A5254] group-hover:bg-[#4C2D33] group-hover:text-white transition-colors">
                <card.Icon className="w-4 h-4" />
              </div>
            </div>
            <div className="text-2xl font-semibold text-[#2C2627]">{card.value}</div>
            <div className="text-xs flex items-center gap-2 font-medium text-[#5A5254] mt-2">
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-[#F5F2F0]">
                {visibleSurveys.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-6 py-12 text-center text-[#8A8284]">
                  ...
                  return (
                    ...
                    >
                      {/* Index */}
                      <td className="pl-6 pr-3 py-4 text-[#8A8284] tabular-nums">
                        {index + 1}
                      </td>

                      {/* Survey */}
                      <td className="pl-3 pr-6 py-4">
                        <div className="font-medium text-[#2C2627]">{survey.title}</div>
                        <div className="text-xs text-[#8A8284] mt-0.5">{t(survey.category)}</div>
                      </td>

                      {/* Responses — progress bar + counts */}
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="relative w-28 h-1.5 bg-[#F5F2F0] rounded-full overflow-hidden">
                            <div
                              className="absolute inset-y-0 left-0 bg-[#4C2D33] rounded-full"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                          <span className="text-xs font-medium text-[#5A5254] tabular-nums">
                      ...
                      {/* Reward */}
                      <td className="px-6 py-4 font-semibold text-[#2C2627] tabular-nums">
                        {formatMnt(survey.rewardMnt)}
                      </td>

                      {/* Length */}
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 text-[#5A5254]">
                      ...
                      {/* Created */}
                      <td className="px-6 py-4 text-[#5A5254] tabular-nums">
                        {format(new Date(survey.createdAt), 'MMM d, yyyy')}
                      </td>

                      {/* Ends */}
                      <td className="px-6 py-4 text-[#8A8284]">
              ...
              <div className="p-6 bg-[#FCFBF9]">
                ...
                <div className="mt-3 p-3 bg-white border border-[#EAE5E3] rounded-md">
                  ...
                  <div className="text-[#8A8284] text-xs mt-1">
                    {t(deletingSurvey.category)} • <span className="font-medium text-[#2C2627]">{formatMnt(deletingSurvey.rewardMnt)}</span> · {deletingSurvey.responsesCurrent}/{deletingSurvey.responsesTarget}
                  </div>
                </div>
