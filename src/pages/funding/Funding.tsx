...
export default function Funding() {
  ...
  const getStatusBadge = (status: Transaction['status']) => {
    switch (status) {
      ...
      default:
        return 'bg-[#F5F2F0] text-[#8A8284] border border-[#EAE5E3]';
    }
  };

  const getTypeBadge = (type: Transaction['type']) => {
    return type === 'Inbound' 
      ? 'text-[#2E7D32] bg-[#E8F5E9] px-2 py-0.5 rounded-sm text-xs font-medium'
      : 'text-[#5A5254] bg-[#F5F2F0] px-2 py-0.5 rounded-sm text-xs font-medium';
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-serif text-[#2C2627]">{t('Funding')}</h1>
          <p className="text-sm text-[#8A8284] mt-1">{t('Manage your accounts, inbound capital, and cash flow.')}</p>
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 px-4 py-2 border border-[#EAE5E3] rounded-md text-sm font-medium text-[#2C2627] hover:bg-[#F5F2F0] transition-colors bg-white shadow-none cursor-pointer">
            <Download className="w-4 h-4" />
            {t('Export CSV')}
          </button>
          <button 
            onClick={() => setIsAddFundsModalOpen(true)}
            className="flex items-center gap-2 px-4 py-2 bg-[#4C2D33] rounded-md text-sm font-medium text-white hover:bg-[#3D2328] transition-colors shadow-none cursor-pointer"
          >
        ...
        {[
          {
            ...
            icon: <ArrowUpRight className="w-4 h-4 text-[#D5C9C6]" />,
            value: '₩29,980,000',
            trend: <span className="text-[#2E7D32] flex items-center gap-0.5"><ArrowUpRight className="w-3 h-3" /> 12.5%</span>,
            subtitle: 'Total 3 transactions'
          },
          {
            title: 'Total Received',
            icon: <Download className="w-4 h-4 text-[#D5C9C6]" />,
            value: '80,520,000 K',
            trend: <span className="text-[#2E7D32] flex items-center gap-0.5"><ArrowUpRight className="w-3 h-3" /> 8.3%</span>,
            subtitle: 'Approx. ₩29,822,222'
          },
          {
            title: 'Avg Exchange Rate',
            icon: <TrendingUp className="w-4 h-4 text-[#D5C9C6]" />,
            value: '2.69',
            trend: <span className="text-[#2E7D32] flex items-center gap-0.5"><ArrowUpRight className="w-3 h-3" /> 0.8%</span>,
            subtitle: 'MMK/KRW'
          },
          {
            title: 'Total Transactions',
            icon: <Plus className="w-4 h-4 text-[#D5C9C6]" />,
            value: '3',
          ...
          <motion.div 
            ...
            className="bg-white rounded-md border border-[#EAE5E3] p-5"
          >
            <div className="text-sm text-[#8A8284] font-medium mb-1 flex justify-between items-center">
              {t(card.title)}
              {card.icon}
            </div>
            <div className="text-2xl font-bold text-[#2C2627] tracking-tight mb-2">{card.value}</div>
            <div className="text-xs flex items-center gap-2 font-medium text-[#5A5254]">
              {card.trend && <>{card.trend}<span className="text-[#D5C9C6]">•</span></>}
          ...
          <div className="flex gap-3 w-full sm:w-auto flex-wrap">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none w-full sm:w-auto pl-4 pr-10 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm font-medium text-[#5A5254] hover:bg-[#F5F2F0] focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33] transition-colors cursor-pointer"
              >
        ...
        <div className="bg-white rounded-md border border-[#F5F2F0] overflow-hidden shadow-none">
                ...
                {isLoading ? (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="font-medium text-[#2C2627]">{trx.description}</div>
                      <div className="text-xs text-[#8A8284] mt-0.5">{trx.id}</div>
                    </td>
                    <td className="px-6 py-4 text-[#5A5254]">{trx.date}</td>
                    <td className="px-6 py-4 text-[#5A5254]">{trx.account}</td>
                    <td className="px-6 py-4 text-[#5A5254]">{t(trx.type)}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 text-[11px] font-medium tracking-wide rounded-md shadow-none ${getStatusBadge(trx.status)}`}>
                        {t(trx.status)}
                      </span>
                    </td>
                    <td className={`px-6 py-4 text-right font-medium ${trx.type === 'Inbound' ? 'text-[#2E7D32]' : 'text-[#2C2627]'}`}>
                      {trx.amount}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button 
                          ...
                          className="p-1.5 text-[#8A8284] hover:text-[#2C2627] hover:bg-[#F5F2F0] rounded-md transition-colors cursor-pointer" 
                          title={t('Edit')}
                        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#F5F2F0] bg-[#FCFBF9]">
            ...
            <div className="flex items-center gap-1">
              ...
              <button className="h-8 min-w-8 px-2 inline-flex items-center justify-center text-sm font-medium border border-[#4C2D33] rounded-md bg-[#4C2D33] text-white tabular-nums cursor-default">
                1
              </button>
              <button className="h-8 min-w-8 px-2 inline-flex items-center justify-center text-sm font-normal border border-[#EAE5E3] rounded-md bg-white text-[#5A5254] hover:bg-[#F5F2F0] transition-colors cursor-pointer tabular-nums">
                2
              </button>
              <button className="h-8 min-w-8 px-2 inline-flex items-center justify-center text-sm font-normal border border-[#EAE5E3] rounded-md bg-white text-[#5A5254] hover:bg-[#F5F2F0] transition-colors cursor-pointer tabular-nums">
                3
              </button>
              <span className="px-1 text-[#8A8284] text-sm">…</span>
              <button className="h-8 min-w-8 px-2 inline-flex items-center justify-center text-sm font-normal border border-[#EAE5E3] rounded-md bg-white text-[#5A5254] hover:bg-[#F5F2F0] transition-colors cursor-pointer tabular-nums">
                24
              </button>
              <button className="h-8 px-3 inline-flex items-center text-sm font-normal border border-[#EAE5E3] rounded-md bg-white text-[#5A5254] hover:bg-[#F5F2F0] transition-colors cursor-pointer">
                {t('Next')}
              </button>
            </div>
          </div>
        ...
        {editingTransaction && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F5F2F0] shrink-0">
              <h2 className="text-lg font-bold text-[#2C2627]">{t('Edit Transaction')}</h2>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Date')}</label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8A8284]" />
                    <input
                      type="text"
                      defaultValue={editingTransaction.date}
                      className="w-full pl-9 pr-3 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33]"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Transaction ID')}</label>
                  <input
                    type="text"
                    defaultValue={editingTransaction.id}
                    disabled
                    className="w-full px-3 py-2 bg-[#F5F2F0] border border-[#EAE5E3] rounded-md text-sm text-[#8A8284] focus:outline-none cursor-not-allowed"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Description')}</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Type')}</label>
                  <div className="relative">
                    <select
                      defaultValue={editingTransaction.type}
                      className="appearance-none w-full px-3 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33] cursor-pointer"
                    >
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Account')}</label>
                  <div className="relative">
                    <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Amount')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Status')}</label>
                  <div className="relative">
                    <select
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#F5F2F0] bg-[#FCFBF9] shrink-0">
              <button
                onClick={() => setEditingTransaction(null)}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#5A5254] bg-white border border-[#EAE5E3] rounded-md hover:bg-[#F5F2F0] transition-colors shadow-none cursor-pointer"
              >
                <X className="w-4 h-4" />
                {t('Cancel')}
              </button>
              <button
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-[#4C2D33] rounded-md hover:bg-[#3D2328] transition-colors shadow-none cursor-pointer"
                onClick={() => {
            ...
            <div className="p-6 bg-[#FCFBF9]">
              ...
              <div className="mt-3 p-3 bg-white border border-[#EAE5E3] rounded-md">
                ...
                <div className="text-[#8A8284] text-xs mt-1">
                  {deletingTransaction.date} • <span className="font-medium text-[#2C2627]">{deletingTransaction.amount}</span>
                </div>
              </div>
      ...
      {isAddFundsModalOpen && (
        ...
        >
          <motion.div 
            ...
            exit={{ scale: 0.95, opacity: 0, y: 10 }}
            transition={{ type: "spring", duration: 0.3 }}
            className="bg-white rounded-md w-full max-w-xl shadow-none border border-[#F5F2F0] flex flex-col max-h-[90vh] overflow-hidden" 
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F5F2F0] shrink-0">
              <h2 className="text-lg font-bold text-[#2C2627]">{t('Add Funds')}</h2>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              <div className="bg-[#FCFBF9] border border-[#EAE5E3] rounded-md p-4 mb-2">
                <p className="text-sm text-[#5A5254] leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Amount')}</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A8284] text-sm">₩</span>
                    <input
                      type="text"
                      placeholder="0.00"
                      className="w-full pl-8 pr-3 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33]"
                    />
                  </div>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Destination Account')}</label>
                  <div className="relative">
                    <select
                      className="appearance-none w-full px-3 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33] cursor-pointer text-[#2C2627]"
                    >
                      <option value="Main Operations">{t('Main Operations')}</option>
                      <option value="Reserve">{t('Reserve')}</option>
                      <option value="Receivables">{t('Receivables')}</option>
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5A5254] mb-1.5">{t('Transfer Method')}</label>
                  <div className="relative">
                    <select
                      className="appearance-none w-full px-3 py-2 bg-white border border-[#EAE5E3] rounded-md text-sm focus:outline-none focus:border-[#4C2D33] focus:ring-1 focus:ring-[#4C2D33] cursor-pointer text-[#2C2627]"
                    >
