...
function ArticleBody({ article }: { article: HelpArticleMeta }) {
  ...
  return (
    <div className="space-y-5 text-sm leading-relaxed text-[#303030]">
      <p>
        {article.description} This guide walks through the end-to-end moderation flow, calls out
        common pitfalls, and links to related admin concepts so you can come back whenever you
        need a refresher.
      </p>

      <h3 className="text-base font-medium text-[#1A1A1A] pt-2">Before you start</h3>
      <p>
        Make sure your admin account has the right role for this action. Most moderation
        actions require <span className="font-medium text-[#1A1A1A]">Moderator</span> or{' '}
        <span className="font-medium text-[#1A1A1A]">Super Admin</span> permissions. Decisions
        are logged to the audit trail and visible to the affected company or respondent.
      </p>

      <div className="bg-[#FFF1EE] rounded-md p-4 flex gap-3">
        <span className="text-[#FF3C21] font-semibold shrink-0">Tip</span>
        <p className="text-[#4A4A4A]">
          Use the Platform Dashboard first thing each morning to spot pending applications,
          waiting payouts, and any respondents flagged for review overnight.
        </p>
      </div>

      <h3 className="text-base font-medium text-[#1A1A1A] pt-2">Step-by-step</h3>
      <ol className="list-decimal list-outside pl-5 space-y-2">
        <li>Open the relevant page from the left sidebar (Companies, Surveys, Respondents, or Payouts).</li>
        <li>Filter by status to scope the list to what actually needs a decision.</li>
        <li>Click a row to open the detail view and verify the context before acting.</li>
        <li>Take the action (approve, pause, reject, suspend) and confirm in the modal.</li>
        <li>The affected party is notified automatically; the event appears on the activity feed.</li>
      </ol>

      <h3 className="text-base font-medium text-[#1A1A1A] pt-2">What to do next</h3>
      <ul className="list-disc list-outside pl-5 space-y-2">
        <li>Check the <span className="font-medium text-[#1A1A1A]">Activity</span> tab on the detail page to see the full event history.</li>
        <li>If a decision is contested, escalate via the audit log — every admin action is timestamped.</li>
        <li>Reversible actions (pause, suspend, reject) can be rolled back from their respective filter tabs.</li>
      </ul>
      ...
      <div className="bg-[#FAFAFA] border border-[#E3E3E3] rounded-md p-4">
        ...
        <p className="text-sm text-[#4A4A4A]">
          Destructive actions stay reversible — suspended companies, rejected surveys, and
          failed payouts can all be restored from their respective filter tabs. Only bulk deletion
          from the audit trail is irreversible, and that requires Super Admin.
        </p>
      </div>
    </div>
  );
    ...
    >
      ...
      {!searchResults && (
        <>
              ...
              {HELP_CATEGORIES.map((cat, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className="text-left bg-white border border-[#E3E3E3] rounded-md p-5 hover:border-[#FFDED5] hover:bg-[#FAFAFA] transition-colors group cursor-pointer"
                  >
          ...
          >
            {[
              { title: 'Moderation playbooks', description: 'Step-by-step admin walkthroughs', Icon: Video },
              { title: 'Policy & release notes', description: 'Updates to moderation rules and the console', Icon: Book },
              { title: 'Contact platform team', description: 'Escalate a case to the iDap team', Icon: MessageSquare },
            ].map((r) => (
              <button
                key={r.title}
                className="flex items-center gap-3 bg-white border border-[#E3E3E3] rounded-md p-4 text-left hover:border-[#FFDED5] hover:bg-[#FAFAFA] transition-colors cursor-pointer group"
              >
